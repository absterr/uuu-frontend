import type { BulkFile } from "@/lib/types/analysis";
...
export default function CodeInputPanel({
  ...
  return (
    <section
      ...
      className={cn(
        "flex min-h-0 min-w-0 flex-1 flex-col border-b border-foreground/10 lg:border-r lg:border-b-0",
        className,
      )}
    >
      <header className="flex items-center justify-between border-b border-foreground/10 px-4 py-3 md:px-6">
        ...
        <div className="flex items-center gap-3">
          {!isBulk && (
            <label
              className={`cursor-pointer border border-foreground/15 px-2.5 py-1
                text-xs font-medium text-foreground/70 hover:border-accent hover:text-plum`}
            >
              Upload file
              <input
                type="file"
      ...
      <div className="flex min-h-0 w-full flex-1 flex-col p-4 md:p-6">
        {isBulk ? (
          <div className="flex min-h-0 flex-1 flex-col gap-4">
            <label
              ...
              className={`flex flex-col items-center justify-center gap-2 border
                border-dashed border-foreground/15 bg-foreground/5 p-6 text-center`}
            >
              <p className="text-sm text-foreground/60">Drag files here, or</p>

              <span
                className={`cursor-pointer border border-foreground/15 px-3 py-1.5
                  text-xs font-medium text-foreground/70 hover:border-accent hover:text-plum`}
              >
                Browse files
              </span>
            ...
            <ul className="min-h-0 flex-1 divide-y divide-foreground/10 overflow-y-auto border-t border-foreground/10">
              {value.map((file) => (
                <li
                  key={file.id}
          ...
          <textarea
            ...
            className={`w-full flex-1 resize-none border border-foreground/15 bg-foreground/5
              p-4 font-mono text-xs text-foreground focus-visible:outline-1 focus-visible:outline-accent`}
          />
        )}
      </div>
      ...
      <footer
        className={cn(
          "flex items-center border-t border-foreground/10 px-4 py-3 md:px-6",
          isBulk ? "justify-between" : "justify-end",
        )}
      >
        {isBulk && (
          <button
        ...
        <button
          ...
          className={`flex cursor-pointer items-center gap-1 bg-plum px-4 py-2
            text-xs font-medium text-background hover:bg-plum/90 disabled:bg-foreground/10
